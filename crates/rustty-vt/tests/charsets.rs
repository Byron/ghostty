...
fn legacy_mapping_keeps_original_width_and_repeat_codepoint() {
    ...
    assert_eq!(
        &*terminal.screen().cell_text(&terminal.screen().rows[0], 3),
        "界"
    );
    assert_eq!(cells[3].width, 2);
    terminal.feed(b"\x1b(0q\x1b(B\x1b[b");
    assert_eq!(
        &*terminal.screen().cell_text(&terminal.screen().rows[0], 5),
        "─"
    );
    assert_eq!(
        &*terminal.screen().cell_text(&terminal.screen().rows[0], 6),
        "q"
    );
    terminal.feed(b"\x1b(0\x1b%Gq");
    assert_eq!(
        &*terminal.screen().cell_text(&terminal.screen().rows[0], 7),
        "─"
    );
}
...
fn wide_spacers_consume_single_shifts() {
    ...
    assert_eq!(
        &*terminal.screen().cell_text(&terminal.screen().rows[1], 2),
        "#"
    );
