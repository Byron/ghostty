            ...
            loop {
                ...
                for byte in bytes {
                    ...
                    if synchronized && !terminal.modes.dec(2026) {
                        ...
                        if screen.rows[0].text() != "stable frame"
                            || screen.rows[1].text() != format!("star {completed}")
                            || !screen.cursor.visible
                            || (screen.cursor.row, screen.cursor.col) != (2, 2)
                        {
                            return Err(format!("batch {completed} ended before its screen/cursor update: text={:?}, cursor=({}, {}, {})", screen.rows[1].text(), screen.cursor.row, screen.cursor.col, screen.cursor.visible).into());
                        }
                    }
                }
            }
