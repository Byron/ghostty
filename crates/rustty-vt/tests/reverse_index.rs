...
fn reverse_index_outside_horizontal_margins_uses_bounded_cursor_up() {
    for column in [1, 8] {
        ...
        let rows = terminal.screen().rows.clone();
        terminal.feed(b"\x1bM");
        assert_eq!(terminal.screen().rows, rows);
...
fn reverse_index_preserves_pending_wrap_when_it_scrolls() {
    ...
    assert_eq!(terminal.screen().rows[1].text(), "abcd");
    assert!(terminal.screen().cursor.pending_wrap);
    terminal.feed(b"X");
    assert_eq!(terminal.screen().rows[1].text(), "Xbcd");
}
